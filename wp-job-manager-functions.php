...
function get_job_listings( $args = array() ) {
	...
	if ( 'rand_ignore_featured_flag' === $args['orderby'] ) {
	...
	if ( apply_filters( 'get_job_listings_cache_results', true ) ) {
		...
		if ( $cached_query ) {
			...
			if ( 'rand' === $args['orderby'] ) {
				usort( $result->posts, '_wpjm_shuffle_post_results_helper' );
			} elseif ( 'rand_ignore_featured_flag' === $args['orderby'] ) {
...
if ( ! function_exists( '_wpjm_shuffle_post_results_helper' ) ) :
	 ...
	 * @return bool
	 */
	function _wpjm_shuffle_post_results_helper( $a, $b ) {
